...
const FRAGMENTS = {
  eye: { owner: 'Soline', digit: '7', icon: '👁', title: "Marque de l’Œil", theme:'eye', whisper:'Le Verre refermé ne laissera plus l’Ombre regarder le village.' },
  water: { owner: 'Sacha', digit: '4', icon: '💧', title: "Marque de l’Eau", theme:'water', whisper:'L’Eau garde la mémoire du courant et efface la trace de l’Ombre.' },
  passage: { owner: 'Vadim', digit: '9', icon: '◠', title: 'Marque du Passage', theme:'passage', whisper:'La Pierre rabat ses arches et ferme la route entre les rives.' },
  iron: { owner: 'Louise', digit: '2', icon: '⚙', title: 'Marque du Fer', theme:'iron', whisper:'Le Fer fixe le dernier verrou et empêche l’appel des lignes mortes.' }
};
...
const ILLUSTRATIONS = {
  ...
  pondPhoto: 'assets/veuxhaulles-etang.png',
  weirPhoto: 'assets/veuxhaulles-seuil.png',
  bridgePhoto: 'assets/veuxhaulles-pont.png',
  towerPhoto: 'assets/veuxhaulles-tour-eau.png'
};
...
function photoReveal(srcKey,title,caption){
  return `<div class="photo-reveal">${illustration(srcKey,title,caption,'real-location')}</div>`;
}
...
function renderLanding(){
  ...
  app.innerHTML=`<section class="landing-page ${locked?'landing-locked':''}">
    <div class="landing-orbit"><span>👁</span><span>💧</span><span>◠</span><span>⚙</span></div>
    <div class="landing-emblem">${locked?'⌛':'✦'}</div>
    <div class="landing-place">Veuxhaulles-sur-Aube</div>
    <h1 class="landing-title">Le Sceau des<br><strong>Quatre Passages</strong></h1>
    <p class="landing-lead">Une aventure géolocalisée où le Verre, l’Eau, la Pierre et le Fer gardent encore la trace de l’Ombre.</p>
    <div class="landing-passages"><span>👁 Verre</span><span>💧 Eau</span><span>◠ Pierre</span><span>⚙ Fer</span></div>
    ${locked?`<div class="lock-panel"><div class="lock-rune">✦</div><div class="lock-kicker">Le Livre refuse encore de s’ouvrir</div><h2>Le sceau est intact.</h2><p>Les quatre noms sont écrits, mais l’encre demeure noire. Le Livre ne répondra qu’au moment choisi.</p><div class="lock-date">Déverrouillage : <strong>${escapeHtml(formatUnlockDate(unlock))}</strong></div><div class="lock-countdown" id="lockCountdown">${countdownText(unlock)}</div><p class="small">Revenez lorsque le compte sera terminé. Le mode Maître du jeu reste accessible avec ⚙.</p></div>`:`<div class="landing-actions">${btn(hasProgress?'Reprendre l’aventure':'Entrer dans le Livre','landingPrimary','good')}${hasProgress?btn('Relire la préface','landingStory','secondary'):''}</div>`}
    ${!locked?(hasProgress?`<div class="landing-resume"><span>Progression sauvegardée</span><strong>${escapeHtml(currentProgressLabel())}</strong></div>`:'<div class="landing-resume"><span>4 Veilleurs · parcours à pied · téléphone géolocalisé</span><strong>Le Livre attend vos quatre noms.</strong></div>'):''}
    <div class="landing-tip">🔊 Son recommandé · Restez groupés pendant toute l’aventure</div>
  </section>`;
  if(locked){startLockCountdown(unlock);return;}
  $('#landingPrimary').onclick=()=>{
    if(hasProgress){ startAmbient(); startGeo(); resumeGameFromState(); }
    else transition('Le Livre vous reconnaît','Avant le premier passage, découvrez l’histoire des anciens Veilleurs',renderHome,'✦');
  };
  $('#landingStory')?.addEventListener('click',()=>transition('Le Livre se rouvre','Relisez le serment avant de reprendre votre chemin',renderHome,'✦'));
}
...
function startAmbient(){
  ...
  const A=getAudio(); if(!A) return;
  try{
    const master=A.createGain(); master.gain.value=.01;
    const f=A.createBiquadFilter(); f.type='lowpass'; f.frequency.value=350;
    master.connect(f); f.connect(A.destination);
    const a=A.createOscillator(),b=A.createOscillator(); a.frequency.value=55;b.frequency.value=82.5;a.type='sine';b.type='triangle';
    a.connect(master);b.connect(master);a.start();b.start();ambientNodes=[a,b,master,f];
  }catch{}
}
function stopAmbient(){ ambientNodes.forEach(n=>{try{n.stop?.()}catch{} try{n.disconnect?.()}catch{}}); ambientNodes=[]; }
...
function fearScreen(stage='default'){
  ...
  el.innerHTML=`<div class="fear-screen-bg" style="background-image:linear-gradient(180deg,rgba(4,4,8,.1),rgba(4,4,8,.88)),url('${s.img}')"></div><div class="fear-screen-vignette"></div><div class="fear-approach"><div class="fear-shadow-haze"></div><div class="fear-shadow-core"></div><div class="fear-shadow-eyes"></div></div><div class="fear-screen-content"><div class="fear-kicker">${escapeHtml(s.kicker)}</div><div class="fear-mark">${escapeHtml(s.mark)}</div><div class="fear-title">${escapeHtml(s.title)}</div><div class="fear-sub">${escapeHtml(s.sub)}</div></div>`;
  document.body.appendChild(el);
  haptic([30,60,100]);
  requestAnimationFrame(()=>el.classList.add('show'));
  setTimeout(()=>el.classList.add('leave'),1450);
...
function omen(title,sub=''){
  ...
  el.innerHTML=`<div class="omen-rune">✦</div><div class="omen-title">${escapeHtml(title)}</div>${sub?`<div class="omen-sub">${escapeHtml(sub)}</div>`:''}`;
  document.body.appendChild(el);requestAnimationFrame(()=>el.classList.add('show'));setTimeout(()=>el.classList.add('leave'),1100);setTimeout(()=>el.remove(),1800);
}
function transition(title,sub,cb,icon='✦'){
  ...
  document.body.appendChild(el);requestAnimationFrame(()=>el.classList.add('show'));revealSound();setTimeout(()=>el.classList.add('leave'),1350);setTimeout(()=>{el.remove();sceneLock=false;cb?.();},2100);
}
function cinematicIntro(cb){
  ...
  document.body.appendChild(el);
  tone(110,.4,'sine',.01);tone(147,.4,'triangle',.012,.35);tone(196,.5,'sine',.014,.8);tone(262,.6,'sine',.016,1.35);
  requestAnimationFrame(()=>el.classList.add('show'));setTimeout(()=>el.classList.add('leave'),3900);setTimeout(()=>{el.remove();sceneLock=false;cb?.();},4500);
}
...
function renderHome(){
  ...
  ${card(`${badge('Livre des Veilleurs','Préface')}${illustration('book','Le Livre des Veilleurs','Quatre passages furent scellés pour empêcher l’Ombre de suivre l’Aube jusqu’au village.')}
    ...
    ${lore('Votre mission','Suivre la trace de l’Ombre à travers Veuxhaulles, récupérer les quatre marques, puis revenir au point du serment pour refermer son chemin en sens inverse.')}
...
function renderPrologue(){
  ...
  <p class="story">Le Livre a retrouvé quatre noms : <strong>Vadim, Louise, Soline et Sacha</strong>. Il ne choisit pas quatre personnes par hasard. Chacun devra réussir ce que les autres ne peuvent accomplir à sa place.</p>
  <div class="symbols role-grid"><div class="sigil"><span>👁</span><small>Soline<br>Œil du Veilleur</small></div><div class="sigil"><span>💧</span><small>Sacha<br>Porte-Eau</small></div><div class="sigil"><span>◠</span><small>Vadim<br>Gardien du Passage</small></div><div class="sigil"><span>⚙</span><small>Louise<br>Gardienne du Fer</small></div></div>
...
function renderGateChurch(){
  renderGate({screen:'gate-church',chapter:'Épreuve I',title:'Les Yeux de Verre',key:'church',next:renderChurch,image:'church',icon:'👁',story:'Le premier passage se trouve au cœur du village. Avant même que l’on parle d’étangs, de pont ou de rail, les habitants jurèrent avoir vu des lueurs changer dans l’église. Les couleurs s’éteignaient, puis une forme sombre semblait regarder Veuxhaulles depuis le verre.',suspenseTitle:'Le verre ne montre pas toujours la lumière',suspenseText:'Approchez de la façade sans chercher une inscription : le premier indice est dans ce que la pierre laisse boire au jour.'});
}
...
function renderFragment(key,next){
  ...
  app.innerHTML=card(`${badge('Fragment retrouvé',f.title)}<div class="seal-recovery-visual seal-theme-${f.theme}"><div class="seal-halo"></div><div class="seal-rings"><span></span><span></span><span></span></div><div class="seal-special"><i></i><i></i><i></i><i></i></div><div class="seal-core">${f.icon}</div></div><h2 class="center">${f.title}</h2>
  ...
  <p class="small center">Mémorisez ce chiffre. Ne le dites pas aux autres avant le dernier rituel.</p>${btn('Fragment mémorisé','fragmentNext','good')}`,'chapter-card fragment-card seal-recovered');
  ...
  $('#fragmentNext').onclick=()=>transition('Une marque de plus',key==='eye'?'L’Ombre a été chassée du Verre et file maintenant vers l’Eau':key==='water'?'Le courant est brisé. L’Ombre cherche désormais une route de pierre':key==='passage'?'Les arches sont fermées. Il ne reste plus que le Fer et ses anciens signaux':'Les quatre marques sont revenues. Il faut rentrer au point du serment',next,f.icon);
}

/* ---------- Épreuve II : étang / seuil ---------- */
function renderGatePond(){
  renderGate({screen:'gate-pond',chapter:'Épreuve II',title:"Le Seuil de l’Eau",key:'pond',next:renderPond,image:'walk',story:"L’Ombre ne resta pas derrière le verre. Chassée de l’église, elle suivit l’Aube jusqu’aux étangs. Les pêcheurs disaient qu’à cet endroit précis, l’eau pouvait devenir blanche sans glace ni neige, comme si elle s’énervait devant une présence cachée.",suspenseTitle:'Écoutez avant de regarder',suspenseText:'Cherchez le lieu où l’eau calme cesse d’être un miroir et franchit un seuil.',safety:'Restez sur les chemins et berges accessibles. Ne descendez jamais sur un ouvrage hydraulique ni au bord d’une chute.'});
}
function renderPond(){
  ...
  $('#pondWordBtn').onclick=()=>{
    if($('#pondWord').value.trim().toUpperCase()==='COURANT'){state.fragments.water=FRAGMENTS.water.digit;complete('pond');successSound();$('#pondMsg').innerHTML=photoReveal('weirPhoto','Le seuil reconnu','Le lieu réel est maintenant inscrit dans le Livre.')+photoReveal('pondPhoto','L’eau derrière le seuil','Une fois l’énigme résolue, le Livre accepte de montrer le paysage tel qu’il est.');setTimeout(()=>renderFragment('water',renderGateBridge),1250);}
...
function renderBridge(){
  ...
  $('#bridgeWordBtn').onclick=()=>{
    if($('#bridgeWord').value.trim().toUpperCase()==='PASSAGE'){state.fragments.passage=FRAGMENTS.passage.digit;complete('bridge');successSound();$('#bridgeMsg').innerHTML=photoReveal('bridgePhoto','Le Passage reconnu','La pierre a livré sa troisième marque.');setTimeout(()=>renderFragment('passage',renderGateTower),900);}
...
function renderTower(){
  ...
  $('#towerWordBtn').onclick=()=>{
    if($('#towerWord').value.trim().toUpperCase()==='CLEF'){state.fragments.iron=FRAGMENTS.iron.digit;complete('tower');successSound();$('#towerMsg').innerHTML=photoReveal('towerPhoto',"Le Gardien du Fer",'La tour est reconnue. Le jeu reste toujours à distance des voies.');setTimeout(()=>renderFragment('iron',renderReturn),1000);}
...
function renderFinal(){
  ...
  <div class="final-order"><span>⚙ Fer</span><span>◠ Passage</span><span>💧 Eau</span><span>👁 Œil</span></div>
  <p>Passez le téléphone d’un Veilleur à l’autre dans cet ordre. Chacun prononce seulement son propre fragment. Entrez les quatre chiffres sans les réordonner ensuite.</p><input class="input final-code" id="finalCode" inputmode="numeric" maxlength="4" placeholder="_ _ _ _"><button class="btn" id="finalBtn">Refermer les quatre passages</button><div id="finalMsg"></div>`,'chapter-card final-card')}</div>`;
  $('#finalBtn').onclick=()=>{
...
function renderTreasure(){
  ...
  <h2 class="center">Vadim · Louise · Soline · Sacha</h2><p class="story center">L’Œil voit de nouveau la lumière. L’Eau retrouve son reflet. Le vieux Passage ne laisse plus entrer l’Ombre. Le Fer est redevenu silencieux.</p><div class="sep"></div><p class="small center">CODE DU COFFRE</p><div class="distance">${escapeHtml(config.treasureCode)}</div><p class="center"><strong>Le trésor des Quatre Veilleurs vous attend.</strong></p>${btn('Refermer le Livre','finishBtn','good')}`,'chapter-card');
